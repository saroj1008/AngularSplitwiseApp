...
export const add_transaction = async (req, res, next) => {
    try {
        ...
        console.log(description, amount, title,req.file)
...
//         console.log(group_id, transaction_id);
//         const { tokenData } = req.body;
...
//added by saroj, replaced professor code
...
export const get_transaction_by_id = async (req, res, next) => {
    try {
        ...
        const results = await groupsModel.aggregate([
            ...
            { $project: { transactions: '$transactions', _id: 0 } },
            { $unwind: '$transactions' },
            { $match: { 'transactions.transaction_id': new Types.ObjectId(transaction_id) } },
        ]);
        ...
        if (results.length > 0) {
            ...
            res.json({ success: false, data: 'Transaction not found' });
        }
    } catch (error) {
        next(error);
    }
